...
namespace Graphs
{
    public class Graph<T>
    {
        ...
        public bool _isAttributed = false;
        ...
        public Node<T> AddNode(T value, string label)
        {
            Node<T> node = new Node<T>() { Data = value, label = label };
            Nodes.Add(node);
            UpdateIndices();
            return node;
        }

        public Node<T> AddNode(T value, int level, bool visited)
        {
            Node<T> node = new Node<T>() { Data = value, level = level, visited = visited };
            Nodes.Add(node);
            UpdateIndices();
            return node;
        }

        public Node<T> AddNode(T value, string label, int level, bool visited)
        {
            Node<T> node = new Node<T>() { Data = value, label = label, level = level, visited = visited };
            Nodes.Add(node);
            UpdateIndices();
            return node;
        }
        ...
        public void AddEdgeAttributed(Node<T> from, Node<T> to, double rFrom = 0, double thetaFrom = 0/*, double rTo = 0, double thetaTo = 0*/)
        {
            from.Neighbors.Add(to);
            if (_isAttributed)
            {
                from.r.Add(rFrom);
                from.theta.Add(thetaFrom);
            }
        ...
        public void RemoveEdge(Node<T> from, Node<T> to)
        {
            ...
            if (index >= 0)
            {
                ...
                if (_isAttributed)
                {
                    from.r.RemoveAt(index);
                    from.theta.RemoveAt(index);
                }
            }
        }
